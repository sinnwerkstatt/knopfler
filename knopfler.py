...
class TinyMatrixtBot():
    def __init__(self):
        ...
        if "CONFIG" in os.environ:
            ...
            config_path = os.path.join(root_path, "knopfler.cfg")
        self.config.read(config_path)
        self.base_url = self.config.get("knopfler", "base_url")
        self.token = self.config.get("knopfler", "token")
        self.connect()
        if len(sys.argv) > 1:
